...
{\centering \includegraphics[width=0.85\linewidth]{strayer_qualifying_exam_files/figure-latex/unnamed-chunk-2-1} 
...
Almost always the loss function is derived from a likelihood function. A
likelihood function is chosen based upon a parametric model that is
assumed to represent the distribution of outcomes for a model with
parameters (weights) \(\mathbb{\theta}\), after conditioning on the
input data.

\BeginKnitrBlock{definition}[Likelihood function]
\protect\hypertarget{def:likelihood}{}{\label{def:likelihood}
\iffalse (Likelihood function) \fi{}
}\[P(\mathbb{\theta}, \mathbb{y}|\mathbb{x}) = P_{\text{model}}(\mathbb{y} | \mathbb{x})\]
\EndKnitrBlock{definition}

For example, with a continuous outcome model it is commonly assumed that
the outcome \(y\) corresponding to input \(\mathbb{x}\) will be normally
distributed. Due to this assumption, the loss function is derived from
the likelihood equation of the normal model.

Optimization then becomes a task of manipulating the parameters of the
network \(\mathbb{\theta}\) such that the predicted conditional
distribution of the output is as close as possible to the observed
conditional distribution. The `distance' from the conditional
distribution is a quantity referred to as the Kullback-Leibler
divergence.

\BeginKnitrBlock{definition}[Expected Kulback-Leibler Divergence]
\protect\hypertarget{def:kld}{}{\label{def:kld} \iffalse (Expected
Kulback-Leibler Divergence) \fi{}
...
\BeginKnitrBlock{definition}[Estimated maximum likelihood loss function]
\protect\hypertarget{def:estnegloglik}{}{\label{def:estnegloglik}
\iffalse (Estimated maximum likelihood loss function) \fi{}
...
\protect\hypertarget{def:mse}{}{\label{def:mse} \iffalse (Expected mean
squared error loss) \fi{}
...
\BeginKnitrBlock{definition}[Expected binary cross entropy loss]
\protect\hypertarget{def:crossentropy}{}{\label{def:crossentropy}
\iffalse (Expected binary cross entropy loss) \fi{}
...
\BeginKnitrBlock{definition}[Expected categorical cross entropy loss]
\protect\hypertarget{def:catcrossentropy}{}{\label{def:catcrossentropy}
\iffalse (Expected categorical cross entropy loss) \fi{}
...
\section{Avoiding Overfitting}\label{avoiding-overfitting}

Neural networks have a large number of parameters or weights that need
to be tuned in order to minimize their loss. Whenever there are this
many knobs to tune a large concern is overfitting to the training data,
leaving the model unable to generalize.

Neural networks have many ways of dealing with overfitting, some common
to almost all machine learning methods, and some specific to neural
networks.
...
\iffalse (L1 penalization) \fi{} }For a model with a \(k\)-dimensional
